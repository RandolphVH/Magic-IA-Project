...
botaoFiltrar.addEventListener('click', function () {
    console.log('clicou no botão de filtrar');
    // passo 3 - pegar os valores dos campos de categoria e preço
    ...
    cartas.forEach(function (carta) {
        ...
        const temFiltoDeCategoria = categoriaSelecionada !== '';

        const cartaNapBateComFiltroDeCategoria =
            categoriaSelecionada.toLowerCase() !== categoriaCarta.toLowerCase();

        if (temFiltoDeCategoria && cartaNapBateComFiltroDeCategoria) {
            mostrarCarta = false;
        }
        const temFiltroDePreco = precoMaximoSelecionado !== '';
        const cartaNaoBateComFiltroDePrecoMaximo =
         cartaBateComFiltroMaximo = parseFloat(precoCarta) > parseFloat(precoMaximoSelecionado);
